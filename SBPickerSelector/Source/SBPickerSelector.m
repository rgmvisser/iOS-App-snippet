...
+ (SBPickerSelector *) pickerWithNibName:(NSString*)nibName {
    ...
    instance.doneButtonTitle = @"Kies";
    instance.cancelButtonTitle = @"Cancel";
    return instance;
}
...
- (void) showPickerOver:(UIViewController *)parent{
    ...
    UITapGestureRecognizer *gestureRecogniser = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideSelector:)];
    gestureRecogniser.numberOfTapsRequired = 1;
    [self.background addGestureRecognizer:gestureRecogniser];
...
- (void)hideSelector:(id)sender {
    
    [self setAction:sender];
